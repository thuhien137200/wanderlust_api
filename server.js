...
app.put('/user', function (req, res) {
    ...
    Connection.query(`SELECT * FROM users where email='${user.email}' and id <> ${user.id}`, function (error, results, fields) {
        if (error) throw error;
        if (!results[0]) {
            Connection.query(`UPDATE users SET user = '${user}' WHERE id = '${user_id}'`, function (error, results, fields) {
                if (error) throw error;
